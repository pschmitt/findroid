...
// DTOs for the Jellyseerr/Seerr API (/api/v1) - the media request manager sitting in front of
...
// series carry `name`/`firstAirDate`. `mediaInfo` is only present when Jellyseerr already tracks
...
/**
 * Jellyseerr's media availability: 1=UNKNOWN, 2=PENDING, 3=PROCESSING, 4=PARTIALLY_AVAILABLE,
...
// `seasons` is polymorphic on the Jellyseerr side (a number array or the string "all") and only
...
// points at a Jellyseerr instance and the API key is valid.
